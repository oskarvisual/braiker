...
type OperatingCostSettings = { enabled: boolean; monthlyCost: string; livingBots: { id: string; name: string; currentCapital: string }[] };
...
type MacroEvent = { id: string; provider: string; title: string; impact: string; startsAt: string; sourceUrl: string; beforeMinutes: number; afterMinutes: number };
...
function humanError(code: string) {
  ...
  if (code === "INVALID_OPERATING_COST_SETTINGS") return "Enable the charge only with a positive USD amount (up to 12 decimal places).";
...
export function Settings({ initialWallets, initialPaperCapital, initialSync, initialOperatingCosts, initialNotifications, notificationEvents, botProfiles: initialBotProfiles, initialMacroEvents }: { initialWallets: Wallet[]; initialPaperCapital: PaperCapital; initialSync: SyncSettings; initialOperatingCosts: OperatingCostSettings; initialNotifications: NotificationSettings; notificationEvents: readonly NotificationEvent[]; botProfiles: BotProfile[]; initialMacroEvents: MacroEvent[] }) {
  ...
  const [macroBeforeMinutes, setMacroBeforeMinutes] = useState(10);
  const [macroAfterMinutes, setMacroAfterMinutes] = useState(15);
  const [showMacroModal, setShowMacroModal] = useState(false);
  const [macroView, setMacroView] = useState<"upcoming" | "past">("upcoming");
  const [macroPages, setMacroPages] = useState({ upcoming: { events: initialMacroEvents, page: 1, hasMore: false }, past: { events: [] as MacroEvent[], page: 1, hasMore: false } });
  const { pushToast } = useToast();
  const visibleMacroPage = macroPages[macroView];
  ...
  async function saveOperatingCosts(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true);
    const response = await fetch("/api/settings/operating-costs", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ enabled: operatingCosts.enabled, monthlyCost: operatingCosts.monthlyCost }) });
    const body = await response.json(); setSaving(false);
    if (!response.ok) { pushToast({ tone: "error", title: "Monthly operating cost was not saved", message: humanError(body.error) }); return; }
    setOperatingCosts((current) => ({ ...current, enabled: body.enabled, monthlyCost: body.monthlyCost }));
    pushToast({ tone: "success", title: "Monthly operating cost saved", message: body.enabled ? "Living bots will be charged virtually on the first day of each New York month." : "Monthly virtual charges are off." });
  }
  ...
  async function addMacroEvent(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true);
    const response = await fetch("/api/settings/macro-events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ title: macroTitle, impact: "HIGH", startsAt: new Date(macroStartsAt).toISOString(), sourceUrl: macroSourceUrl, beforeMinutes: macroBeforeMinutes, afterMinutes: macroAfterMinutes }) });
    const body = await response.json(); setSaving(false);
    if (!response.ok) { pushToast({ tone: "error", title: "Macro event was not saved", message: humanError(body.error) }); return; }
    setMacroPages((current) => ({ ...current, upcoming: { ...current.upcoming, events: [...current.upcoming.events, body].sort((left, right) => left.startsAt.localeCompare(right.startsAt)) } })); setMacroTitle(""); setMacroStartsAt("");
    pushToast({ tone: "success", title: "High-impact macro event saved", message: `New BUYs will be blocked from ${body.beforeMinutes} minutes before through ${body.afterMinutes} minutes after it.` });
  }
  async function openMacroCalendar(view: "upcoming" | "past" = "upcoming", page = 1) {
    setSaving(true);
    const response = await fetch(`/api/settings/macro-events?view=${view}&page=${page}`, { cache: "no-store" });
    const body = await response.json(); setSaving(false);
    if (!response.ok) { pushToast({ tone: "error", title: "Calendar could not be loaded", message: humanError(body.error) }); return; }
    setMacroPages((current) => ({ ...current, [view]: { events: body.events, page: body.page, hasMore: body.hasMore } })); setMacroView(view); setShowMacroModal(true);
  }
  ...
  return <>
    ...
    <div className="settingsGrid">
      ...
      <section className="panel settingsPanel"><div><p className="eyebrow">VIRTUAL OPERATING COSTS</p><h2>Monthly platform cost</h2></div><p className="muted">Optional virtual rent only. It never transfers money at Alpaca and does not count against a bot’s daily trading-loss limit.</p><form className="settingsControl" onSubmit={saveOperatingCosts}><label className="toggleRow"><input type="checkbox" checked={operatingCosts.enabled} onChange={(event) => setOperatingCosts((current) => ({ ...current, enabled: event.target.checked }))} /><span><strong>Charge living bots monthly</strong><small>On the first day of each New York month, ACTIVE bots — including OFF bots — share the configured cost in proportion to current virtual capital.</small></span></label><label>Monthly operating cost (USD)<input type="number" min="0.01" step="0.01" value={operatingCosts.monthlyCost} onChange={(event) => setOperatingCosts((current) => ({ ...current, monthlyCost: event.target.value }))} disabled={!operatingCosts.enabled} required={operatingCosts.enabled} /></label><small className="muted">Current preview: {operatingCosts.livingBots.length ? `${operatingCosts.livingBots.length} living bot${operatingCosts.livingBots.length === 1 ? "" : "s"} will share the next charge. A bot never pays from capital reserved for an open order.` : "No living bot currently has capital available for a charge."}</small><button type="submit" disabled={saving}>{saving ? "Saving…" : "Save monthly cost settings"}</button></form></section>
    </div>
    <section className="panel settingsPanel macroCalendarPanel"><div className="panelHeading"><div><p className="eyebrow">MACRO SAFETY GUARD</p><h2>High-impact economic calendar</h2><p className="muted">Each event defines its own BUY-protection window. SELLs remain available to reduce risk.</p></div><button type="button" className="compactButton" onClick={() => void openMacroCalendar()}>View calendar</button></div><form className="settingsControl" onSubmit={addMacroEvent}><label>Event title<input value={macroTitle} onChange={(event) => setMacroTitle(event.target.value)} minLength={2} maxLength={255} placeholder="US CPI release" required /></label><label>Start time (your local time)<input type="datetime-local" value={macroStartsAt} onChange={(event) => setMacroStartsAt(event.target.value)} required /></label><label>Minutes before event<input type="number" min="10" max="240" value={macroBeforeMinutes} onChange={(event) => setMacroBeforeMinutes(Number(event.target.value))} required /></label><label>Minutes after event<input type="number" min="15" max="240" value={macroAfterMinutes} onChange={(event) => setMacroAfterMinutes(Number(event.target.value))} required /></label><label>Official source URL<input type="url" value={macroSourceUrl} onChange={(event) => setMacroSourceUrl(event.target.value)} required /></label><button type="submit" disabled={saving}>{saving ? "Saving…" : "Add high-impact event"}</button></form></section>
