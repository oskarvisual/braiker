...
import { AssetUniversePanel } from "@/components/asset-universe-panel";

export default async function SettingsPage() {
  const user = await currentUser();
  if (!user) redirect("/login");
  if (user.role !== "ADMIN") redirect("/");
  const [wallets, paperCapital, task, notificationSettings, telegramSession, botProfiles, macroEvents, operatingCostSettings, livingBots] = await Promise.all([
    ...
    prisma.macroCalendarEvent.findMany({ where: { startsAt: { gte: new Date() } }, orderBy: { startsAt: "asc" }, take: 25 }),
    prisma.operatingCostSettings.findUnique({ where: { scope: "global" } }),
    prisma.botInstance.findMany({ where: { lifeStatus: "ACTIVE" }, select: { id: true, name: true, currentCapital: true }, orderBy: { name: "asc" } })
  ]);
