...
| Operations | `ScheduledTask`, `JobRun`, `SystemEvent`, `ErrorEvent`, `NotificationSettings`, `NotificationAlert`, `AiRuntimeState`, `WorkerRuntimeState`, `TelegramManagerSession`, `TelegramPairingCode`, `TelegramRuntimeState`, `TelegramManagerMessage`, `ManagerChatSession`, `ManagerChatMessage`, `ManagerActionProposal`, `BotChatSession`, `BotChatMessage` |
...
| Bot Manager | `/manager`, `/api/manager/sessions`, `/api/manager/actions` | Admin-only operational chat. Natural-language messages remain read-only; only a single-use, explicit ON/OFF proposal can reach the existing locked control service after human confirmation. |
...
| Bot chat | `/bots/[botId]`, `/api/bots/[botId]/chat` | User/bot-isolated explanation transcript; no configuration, capital, risk, Kill Switch, or order authority. |
...
| `OPENAI_API_KEY`, `OPENAI_MODEL`, `BOT_MANAGER_CHAT_MODEL`, `OPENAI_TIMEOUT_MS`, `AI_MAX_ANALYSES_PER_BOT_PER_DAY` | when AI enabled | Server-only OpenAI advisory configuration; `BOT_MANAGER_CHAT_MODEL` optionally overrides the read-only Manager model, requests use structured output or `store: false`, and no browser receives credentials |
| `SMTP_*` | optional | SMTP delivery for the alert types explicitly selected in Settings; no SMTP secret is stored in MySQL |
| `TELEGRAM_BOT_TOKEN` | optional | Server-only Telegram Bot API token for selected alert delivery and the worker-polled, read-only Bot Manager session; it is never stored in MySQL or returned by APIs |
