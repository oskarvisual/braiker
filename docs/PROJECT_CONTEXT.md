...
- `/manager` — Admin-only **Bot Manager**. It calls Alpaca’s exchange-aware clock for the operational context; failure is explicitly `UNKNOWN`. Exact bot names receive bounded orders/fills, risk decisions, scans, and positions. `Recordar para <nombre exacto>: <regla>` records versioned internal caution. `Evento macro: título | fecha ISO con zona | https://fuente` creates a HIGH event only where the host matches an active approved MACRO Resource. In bot chat, `Recordar: <regla>` does the same. Daily, weekly, and monthly paper-only reports describe their full preceding 24-hour, 7-day, or 30-day window with bot state, orders, aggregate scan outcomes, all persisted daily-briefing counts and bot inputs, reviewed source citations, created/upcoming HIGH macro guards, and sampled latest bot activity; they are bounded, sanitized summaries with separated readable sections and one citation per line, not raw Resource content. An explicit `system report`, `reporte de sistema`, `/status`, or `/report` returns that same daily operational report alongside the current safe System status (MySQL, worker, market stream, Alpaca Paper, OpenAI and configured delivery states) without invoking the AI model. Visible Additional instructions always take priority; internal learning can only caution/veto optional AI review and cannot create signals, change capital/risk/size, alter bot power, or submit orders.
