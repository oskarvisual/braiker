...
- `/login` — sign in.
- `/` — dashboard: for an Admin, one global Alpaca Paper account chart, positions, and broker orders alongside the fleet of every virtual wallet. Bot liquid capital is the sum of each bot's current virtual cash, while unassigned/managed totals remain virtual-wallet accounting. Invalid zero-equity startup snapshots are excluded from the chart and seven-day P&L. No decorative bot initial is shown in the fleet list.
- `/setup` — Admin bot listing and add/edit/clone modal. It shows current bot capital, a read-only survival indicator, risk limits, globally enabled symbols, and one ON/OFF switch. The editor offers an opt-in **Adaptive risk limits** checkbox; when enabled, it hides the three manual fields and permits only deterministic survival reductions inside their saved ceiling. The history modal adds an **Adaptive risk** audit tab for its effective posture changes. Active bots can move capital to/from their wallet; dead bots expose history and cloning only. Clicking a bot name or eye icon opens its full-width history modal with the same survival explanation plus separate Operations and Analysis activity tabs. Settings synchronizes Alpaca's active, tradable, non-OTC US-equity master list into separate paginated **Selected assets** and **Available assets** lists; each list searches on the server so a large refresh cannot hide an enabled symbol. A global disable removes that symbol from every bot, while a later enable leaves watchlists unassigned until an Admin selects it again.
