...
export async function PATCH(request: Request, context: { params: Promise<{ botId: string }> }) {
  try {
    ...
    if (!body.success || (!body.data.name && !body.data.templateId && !body.data.symbols && body.data.customInstructions === undefined && body.data.adaptiveRiskEnabled === undefined && !body.data.riskLimits)) return NextResponse.json({ error: "Invalid bot update" }, { status: 400 });
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      return tx.botInstance.update({ where: { id: bot.id }, data: { name: body.data.name, riskPolicy, ...(body.data.adaptiveRiskEnabled === undefined ? {} : { adaptiveRiskEnabled: body.data.adaptiveRiskEnabled }), ...(template ? { templateId: template.id, avatarSeed: template.avatar } : {}), strategyProfile: { ...priorStrategy, ...(template?.strategyProfile ?? {}), strategyId: "trend-v1", version: 1, templateId: template?.id ?? bot.templateId, customInstructions: body.data.customInstructions ?? priorStrategy.customInstructions ?? "" } } });
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    await prisma.auditLog.create({ data: { userId: user.id, walletId: bot.walletId, action: "BOT_UPDATED", target: botId, metadata: body.data.adaptiveRiskEnabled === undefined ? undefined : { adaptiveRiskEnabled: body.data.adaptiveRiskEnabled } } });
    return NextResponse.json({ id: updated.id, name: updated.name, templateId: updated.templateId, avatarSeed: updated.avatarSeed, adaptiveRiskEnabled: updated.adaptiveRiskEnabled, riskPolicy: updated.riskPolicy });
