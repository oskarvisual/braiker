...
export async function GET(_: Request, context: { params: Promise<{ botId: string }> }) {
  try {
    ...
    const [proposals, adjustments, operatingCosts] = await Promise.all([
      prisma.tradeProposal.findMany({ where: { botId }, select: { id: true } }),
      prisma.botRiskAdjustment.findMany({ where: { botId }, orderBy: { createdAt: "desc" }, take: 50 }),
