...
- `/settings` contains the high-impact economic calendar. Its persistent calendar button opens paginated active/future events and independently paginated immutable past history. Each event requires timestamp and HTTPS URL. The deterministic `MACRO_EVENT_GUARD` blocks only a new `BUY`; it does not stop a risk-reducing `SELL`. Its global window defaults to 10 minutes before/15 after and cannot be reduced below those floors (maximum 240 minutes each).
