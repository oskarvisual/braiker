...
| Settings | `/settings`, `/api/wallets`, `/api/wallets/[walletId]/capital`, `/api/settings/paper-capital`, `/api/settings/synchronization`, `/api/settings/notifications`, `/api/settings/telegram/pairing`, `/api/settings/macro-events`, `/api/settings/assets` | global Paper capital, serialized virtual-wallet budget changes, schedule, persisted alert preferences, server-synchronized tradable-asset catalog, per-event macro protection windows and immutable event history |
| Bot Manager | `/manager`, `/api/manager/sessions`, `/api/manager/actions` | Admin-only operational chat. It calls Alpaca clock per context and supplies bounded exact-bot history. Confirmed exact-name ON/OFF remains the only power mutation. Explicit internal-learning and approved-source macro commands are Admin-only, fully audited, and cannot affect strategy/risk/execution. |
