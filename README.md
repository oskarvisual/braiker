...
- Order-first History across every wallet accessible to the signed-in user, with wallet, lifecycle, and bot filters; selecting a wallet narrows the bot list and keeps its external Alpaca orders visible. It also has a per-bot modal with separate **Operations** and **Analysis activity** tabs. Each modal exposes a deterministic, read-only survival indicator with native emoji, color, an accessible score bar, and the actual operational state. The score is a presentation of the existing survival band, not P&L, a risk override, or a reason to value low liquid cash as failure while capital is invested or reserved. There is no per-bot valuation history yet, so no survival trend is implied. Analysis records explain completed, skipped, and failed cycles in plain English even when no order is placed; retained scan activity is pruned after 30 days.
