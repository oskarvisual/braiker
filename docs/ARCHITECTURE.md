...
- The leased `market-cycle` wakes once per minute and processes all eligible bots in one global Paper-account pass; it must never become a cron job per bot or per virtual wallet.
...
| Execution/portfolio | `MarketEvaluation`, `ExecutionJob`, `Order`, `Fill`, `BotPosition`, `Position`, `BrokerOrderSnapshot`, `PortfolioSnapshot`, `BotPerformanceSnapshot`, `DailyPerformance`, `ReconciliationRun` |
| Operations | `ScheduledTask`, `JobRun`, `SystemEvent`, `ErrorEvent`, `NotificationSettings`, `NotificationAlert`, `AiRuntimeState`, `WorkerRuntimeState`, `MacroCalendarEvent`, `MacroGuardSettings`, `OperatingCostSettings`, `OperatingCostAllocation`, `BotLearnedInstruction`, `TelegramManagerSession`, `TelegramPairingCode`, `TelegramRuntimeState`, `TelegramManagerMessage`, `ManagerChatSession`, `ManagerChatMessage`, `ManagerActionProposal`, `BotChatSession`, `BotChatMessage`, `BotDailyContext` |
...
| Bots | `/setup`, `/api/bots`, `/api/bots/[botId]`, `/api/bots/[botId]/control`, `/api/bots/[botId]/capital`, `/api/bots/[botId]/history`, `/api/bots/[botId]/analysis` | UI control body is `TURN_ON` or `TURN_OFF` only; Admin capital adjustments are serialized and Operations/Analysis history is authorized per wallet. The two history endpoints accept a positive `page` query and return at most 25 records plus look-ahead indicators; history returns independent indicators for orders, adaptive-risk adjustments, and operating costs, while analysis returns the scan indicator. The client offers older pages only in the matching tab. The history read model includes active-position count and reserved capital solely for the deterministic, read-only survival indicator; it is never an execution input. It additionally returns a read-only performance view: first `ALLOCATION`, net `ALLOCATION`/`TOP_UP`/`WITHDRAWAL` flows, current liquid/asset/equity values, realized/unrealized P&L, virtual operating costs, and the latest 90 daily `BotPerformanceSnapshot` observations in chronological order. The allocation migration backfills an immutable event for bots created before capital events existed. |
...
| Bot Manager | `/manager`, `/api/manager/sessions`, `/api/manager/sessions/[sessionId]`, `/api/manager/actions` | Admin-only operational chat. It calls Alpaca clock per context and supplies a bounded exact-bot modal read model: configuration/capital, valued assets, performance and 90 daily equity points, orders/fills, scans, operating costs, and adaptive-risk adjustments. Exact-name Spanish/English trade-cap and capital questions are intercepted before AI and read a typed status: New York-day non-`HOLD` proposal count, adaptive-aware effective cap, and Decimal virtual/reserved/available capital. The worker uses the same New York-day boundary for its `MAX_TRADES_PER_DAY` input. Manager and bot-chat message APIs return a newest bounded page plus a cursor for older messages; the client transcript owns its scroll position. `PATCH` may rename or soft-archive only an owned ordinary session; the pinned Operations session is rejected and archived sessions are omitted from active lists. Explicit `system report` / `reporte de sistema` / `/status` / `/report` is handled deterministically, reads the same sanitized `getSystemStatus` dependencies plus a read-only daily report window, and never calls the model or mutates state beyond its chat transcript. Confirmed exact-name ON/OFF remains the only power mutation. Explicit internal-learning and approved-source macro commands are Admin-only, fully audited, and cannot affect strategy/risk/execution. |
...
| Bot chat | `/bots/[botId]`, third history-modal **Chats** tab, `/api/bots/[botId]/chat`, `/api/bots/[botId]/chat/sessions/[sessionId]` | Web-only user/bot-isolated sessions; they are locked unless the bot is alive, `PAPER_ACTIVE`, `RUNNING`, and not Kill-Switched. The API returns newest-first bounded storage pages in chronological client order and cursor-paginates older messages. Exact capital/trade-cap questions use the same deterministic typed status as Manager before AI. `PATCH` requires existing wallet authorization and may only rename or soft-archive the owner’s unarchived session. **Ask in chat** creates a new session then passes a user-authorized, bot-scoped exact order (with proposal/risk/fills) or scan record as read-only focus context. No configuration, capital, risk, Kill Switch, or order authority. Explicit `Important:` / `Importante:` user context and Manager-created notes persist separately for the current NY market day only. |
...
| History | `/activity`, `/bots/[botId]` | wallet, order-lifecycle, and per-bot filtering across all wallets accessible to the signed-in user; choosing a wallet narrows the available bots while retaining its unlinked Alpaca snapshots; internal orders are linked by immutable bot id, so a bot rename does not affect attribution |

`/status` is an Admin-only server-rendered operational view. It checks MySQL with `SELECT 1`, reads the durable `WorkerRuntimeState` written by the worker (never a container-local file), checks the durable authenticated Alpaca market-stream heartbeat, and calls the existing Alpaca Paper adapter health check. It also shows bot lifecycle counts. ON means eligible for the worker market cycle, not proof that the bot is currently evaluating; a stale or unavailable worker makes this explicit. The worker owns exactly one WebSocket connection for the union of active enabled watchlists plus `SPY`/`QQQ`; it persists completed minute bars and low-volume lifecycle/heartbeat diagnostics, never raw quote ticks, updates subscriptions, and reconnects with bounded backoff, while the REST cycle provides reconciliation/backfill and the idempotent evaluation boundary. `MarketStreamEvent` has a provider/type/time index for the heartbeat lookup. OpenAI is `Configured` only when the optional advisory feature flag and server-only key are present; a durable provider quota/billing rejection changes that service to a sanitized warning and sets `AiRuntimeState` to `QUOTA_EXHAUSTED`. Only that persistent reason exposes the Admin reactivation control. It sends one minimal non-trading Responses call and reopens the circuit only after success; other failures remain closed. SMTP, webhook, and Telegram entries accurately describe configuration readiness. Secrets and destinations remain server-only.

After each worker heartbeat and once per minute, the operational monitor samples the status. Only required dependency warnings/unavailability (`database`, `worker`, `market-stream`, `alpaca`) create a `SYSTEM_STATUS_FAILURE` alert; intentionally disabled optional email/webhooks/Telegram never create one. Each `NotificationAlert` has one unique cause key, opens or refreshes atomically, resolves when the cause recovers, and resets its delivery state for a later recurrence. Bot Manager reports are scheduled through MySQL leases: a pre-market daily report at 08:35 ET, an early-close candidate at 13:05 ET, a regular-close candidate at 16:05 ET, then weekly Friday 16:45 ET and monthly 17:00 ET on the first day. Pre-market and closing candidates require that day's immutable exchange-session briefing, so weekends and exchange holidays are skipped. The closing candidates consult Alpaca's clock: an open exchange skips the 13:05 candidate, a closed exchange publishes; the idempotent daily key prevents duplicate early/regular-close reports. Pre-market summarizes the previous completed weekday through pre-market, closing summarizes only the current New York market day, and weekly/monthly retain 7-day/30-day windows. The selected `BOT_MANAGER_DAILY_REPORT` preference deliberately delivers both pre-market and closing reports, preserving existing channel selections. All reports query fleet state, order outcomes, aggregate scans and no-scan bots, persisted daily-briefing counts/bot-input total with bounded source category/title citations, HIGH macro events created in the period, upcoming HIGH guards within 48 hours, and sampled latest bot scan state. The delivery text bounds and normalizes those display values into separated readable sections, with one bounded source citation per line; it never includes raw Resource extraction, URLs, credentials, or broker payloads. An explicit Manager system-report request calls `getSystemStatus` and reads the non-persisting DAILY report input in parallel; it returns both sanitized views through the existing chat only, does not create a notification alert, and never calls the AI model. Webhook and email delivery are opt-in per selected event. Telegram delivery additionally requires `NotificationSettings.telegramManagerEnabled`, a persisted master switch that defaults false; this gate prevents all Telegram alert/report delivery before a channel attempt is claimed. The report upsert preserves completed delivery timestamps, so a scheduler retry does not redeliver a completed channel. The notification-settings API validates each channel independently only when it is effectively enabled; incomplete details in an inactive channel cannot block saving a separate active channel. Webhook destinations are decrypted only in the worker; SMTP and the Telegram bot token stay entirely in environment variables. Telegram alerts and report notifications are delivered only to the paired Bot Manager session and plans/status/API responses expose no chat identifier. Delivery payloads contain only sanitized alert fields. OpenAI cannot expose a reliable remaining-account balance through the application key, but a provider `insufficient_quota`/billing rejection opens a distinct `OPENAI_QUOTA_EXHAUSTED` alert and durable `AiRuntimeState` circuit. The circuit prevents future advisory calls until the Admin-only recheck successfully completes; a failed or still-quota-limited recheck remains closed. A failed worker or unavailable database cannot reliably send its own alert, so an independent monitor must poll public `/api/status` and alert on a non-healthy response. These alerts and the AI circuit are observational/control-plane only: they never change bot power, capital, risk, Kill Switch, or execution authority.

Telegram is worker-owned and uses `getUpdates` polling with a durable `TelegramRuntimeState.nextUpdateId`; a restart does not replay a processed update. Its master switch, pairing, and inbound-message gates are all enforced before `sendManagerMessage`, so Manager learning and macro commands work only for a paired Admin with inbound messages enabled. The Manager calls Alpaca clock separately from AI: an error becomes `UNKNOWN`. Exact-name history is bounded. `Recordar para <nombre exacto>: <regla>` writes an auditable `BotLearnedInstruction` revision, separate from the visible strategy instruction; the latter wins conflicts. `Evento macro: título | fecha ISO con zona | https://fuente` accepts only a future HTTPS timestamped HIGH event whose host is an active approved MACRO `ResourceSource`, writes the actor/source audit, and deduplicates at the event unique key. Each `MacroCalendarEvent` has its own validated 10/15-minute-minimum BUY protection window, which the worker and risk guard read directly; `MacroGuardSettings` remains a retired compatibility table only. These paths can only add caution/veto to optional AI review or block new BUYs. They never form signals, loosen risk, change capital, alter size, or submit orders.

`BotLearningProposal` is a separate approval queue. The reconciliation path considers only persisted negative realized SELL fills; three for the same bot/symbol in the last 30 days create at most one deduplicated seven-day proposal with fill identifiers as evidence. It never creates a `BotLearnedInstruction` itself. If Telegram is paired, enabled for inbound messages, and its selected events contain `LEARNING_PROPOSAL`, a durable `NotificationAlert` delivers the proposal to Telegram; otherwise a per-bot `ManagerChatSession` is created/reused for each global Admin. Both web and Telegram resolution atomically claim a pending proposal. Only an Admin `APPROVE` records the resulting caution-only internal revision; `REJECT`, expiry, duplicates, and a non-Admin have no behavioral effect.
...
The Admin-only asset-catalog synchronization calls the server-side Alpaca Paper assets endpoint and persists active, tradable, non-OTC US equities in bounded atomic SQL batches. A later batch failure never rolls back already completed batches: the API returns only the safe unavailable code plus the completed count, never an Alpaca, Prisma, or database error. The browser reloads the catalog and presents that safe partial result next to the synchronization action.
...
After terminal order reconciliation, each global Paper synchronization calculates every bot's attributed equity from its current liquid capital plus the same synchronized asset valuation. It upserts one `BotPerformanceSnapshot` per bot/New York market day, replacing that day's point only when every attributed open position has a current Alpaca valuation. The history read model never invents a missing point from entry price. `BotPerformanceSnapshot` is observational and must not be read by risk, capital allocation, survival, or execution code.
...
| `TELEGRAM_BOT_TOKEN` | optional | Server-only Telegram Bot API token that makes Telegram available only; the persisted Settings master switch must also be enabled before polling, pairing, alerts, reports, or Bot Manager mirroring can occur. It is never stored in MySQL or returned by APIs |
...
After each open-market pass, the worker writes one `BotScanRun` per active bot. It contains only an operator-safe summary and per-symbol outcome (`HOLD`, `APPROVED`, risk/AI rejection, insufficient bot capital, duplicate candle, or missing market data); raw exception strings, credentials, and provider payloads are never stored there. On the first closed-market pass, the worker records the safe `MARKET_CLOSED` wait, then writes Alpaca's exchange-aware `next_open` to `ScheduledTask.nextRunAt` for `market-cycle`. `runTask` respects that durable future time, so no more closed-market cycles or scan rows are created before the next open, including over weekends and exchange holidays. An open pass clears the deferment. Empty-watchlist waits remain rate-limited. The worker runs leased daily retention for `BotScanRun` rows older than 30 days and replaceable market telemetry: `MarketStreamEvent` after 7 days, `MarketBar` after 14 days, and `MarketEvaluation` after 30 days. Retention indexes those timestamp predicates. It does not delete `MarketSnapshot`, `StrategySignal`, `TradeProposal`, `RiskDecision`, `Order`, or `Fill`, preserving decision and financial evidence. `GET /api/bots/[botId]/analysis` is authenticated, enforces the same wallet-membership rule as order history, and pages scan rows in 25-record chunks.
...
The worker also owns one persistent Alpaca market-data WebSocket for the union of ON-bot watchlists plus `SPY`/`QQQ`, never the browser and never one socket per bot. It authenticates after connection, updates one-minute-bar and quote subscriptions as watchlists change, persists only completed bars plus low-volume lifecycle events, and reconnects with bounded exponential backoff. Quote ticks remain transient because the REST cycle obtains the latest quote for each evaluation. It writes a durable authenticated heartbeat that `/status` and `/api/status` read. REST backfill remains active for gaps and the `MarketEvaluation` key remains the decision idempotency boundary, so streaming never bypasses risk or execution. Global reconciliation is executed once per cycle; broker orders with a BrAIker `client_order_id` are attributed to the originating virtual wallet through the internal order → proposal → bot trace. Broker-only orders remain associated with the default global wallet.
