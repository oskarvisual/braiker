...
- `/manager` — Admin-only **Bot Manager**. It has one pinned Operations transcript plus ordinary browser text conversations. It explains sanitized persisted operations; natural-language messages remain non-executing. Telegram may only prepare a single explicit `/on <bot-id>` or `/off <bot-id>` proposal; `CONFIRMAR <código>` is short-lived/single-use and executes through the ordinary locked bot control, revalidating the bot at execution. No chat can alter capital, risk, Kill Switches, settings, users, or orders.
...
3. A broader Manager action surface. The only implemented action workflow is explicit one-bot Telegram ON/OFF proposals, with one-time confirmation and locked execution revalidation; conversations cannot modify budget, limits, Kill Switches, instructions, or orders.
4. A durable high-impact economic-calendar feed and the macro execution guard. Daily bot inputs are conservative advisory context, not a risk/execution authorization; the calendar feed and deterministic risk rejection remain a separate implementation.
