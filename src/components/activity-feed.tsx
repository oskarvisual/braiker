...
export function ActivityFeed({ orders, bots, wallets }: { orders: HistoryOrder[]; bots: Array<{ id: string; name: string; walletId: string }>; wallets: Array<{ id: string; name: string }> }) {
  ...
  return <>
    ...
    <section className="panel simpleTablePanel historyPanel"><div className="historyFilters"><label>Wallet<select value={walletId} onChange={(event) => { setWalletId(event.target.value); setBotId("ALL"); }}><option value="ALL">All wallets</option>{wallets.map((wallet) => <option key={wallet.id} value={wallet.id}>{wallet.name}</option>)}</select></label><label>Order status<select value={state} onChange={(event) => setState(event.target.value as "ALL" | OrderHistoryState)}><option value="ALL">All statuses</option>{historyStates.map((entry) => <option key={entry} value={entry}>{stateLabels[entry]}</option>)}</select></label><label>Bot<select value={botId} onChange={(event) => setBotId(event.target.value)}><option value="ALL">All bots</option><option value="UNASSIGNED">No bot / external</option>{visibleBots.map((bot) => <option key={bot.id} value={bot.id}>{bot.name}</option>)}</select></label><label>Find symbol<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="AAPL, buy, filled…" /></label></div>
