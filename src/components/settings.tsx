...
export function Settings({ initialWallets, initialPaperCapital, initialSync, initialNotifications, notificationEvents, botProfiles: initialBotProfiles, initialMacroEvents, initialMacroGuard }: { initialWallets: Wallet[]; initialPaperCapital: PaperCapital; initialSync: SyncSettings; initialNotifications: NotificationSettings; notificationEvents: readonly NotificationEvent[]; botProfiles: BotProfile[]; initialMacroEvents: MacroEvent[]; initialMacroGuard: { beforeMinutes: number; afterMinutes: number } }) {
  ...
  async function saveMacroGuard(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true);
    const response = await fetch("/api/settings/macro-guard", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(macroGuard) });
    const body = await response.json(); setSaving(false);
    if (!response.ok) { pushToast({ tone: "error", title: "Macro guard was not saved", message: humanError(body.error) }); return; }
    setMacroGuard(body); pushToast({ tone: "success", title: "Macro protection window saved" });
  }
  async function openMacroCalendar(view: "upcoming" | "past" = "upcoming", page = 1) {
    ...
    setMacroEvents(body.events); setMacroView(view); setMacroPage(body.page); setMacroHasMore(body.hasMore); setShowMacroModal(true);
  }
  ...
  return <>
    ...
    <section className="panel settingsPanel macroCalendarPanel"><div className="panelHeading"><div><p className="eyebrow">MACRO SAFETY GUARD</p><h2>High-impact economic calendar</h2><p className="muted">New BUYs are blocked from {macroGuard.beforeMinutes} minutes before through {macroGuard.afterMinutes} minutes after a HIGH event. SELLs remain available to reduce risk.</p></div><button type="button" className="compactButton" onClick={() => void openMacroCalendar()}>View calendar</button></div><form className="settingsControl" onSubmit={saveMacroGuard}><label>Minutes before event<input type="number" min="10" max="240" value={macroGuard.beforeMinutes} onChange={(event) => setMacroGuard((current) => ({ ...current, beforeMinutes: Number(event.target.value) }))} required /></label><label>Minutes after event<input type="number" min="15" max="240" value={macroGuard.afterMinutes} onChange={(event) => setMacroGuard((current) => ({ ...current, afterMinutes: Number(event.target.value) }))} required /></label><button type="submit" disabled={saving}>{saving ? "Saving…" : "Save protection window"}</button></form><form className="settingsControl" onSubmit={addMacroEvent}><label>Event title<input value={macroTitle} onChange={(event) => setMacroTitle(event.target.value)} minLength={2} maxLength={255} placeholder="US CPI release" required /></label><label>Start time (your local time)<input type="datetime-local" value={macroStartsAt} onChange={(event) => setMacroStartsAt(event.target.value)} required /></label><label>Official source URL<input type="url" value={macroSourceUrl} onChange={(event) => setMacroSourceUrl(event.target.value)} required /></label><button type="submit" disabled={saving}>{saving ? "Saving…" : "Add high-impact event"}</button></form></section>
    ...
    {showMacroModal && <div className="modalOverlay" role="presentation"><section className="modalCard" role="dialog" aria-modal="true" aria-labelledby="macro-calendar-modal-title"><div className="modalHeading"><div><p className="eyebrow">MACRO SAFETY GUARD</p><h2 id="macro-calendar-modal-title">{macroView === "past" ? "Past events" : "Active and future events"}</h2></div><button type="button" className="iconButton" onClick={() => setShowMacroModal(false)} aria-label="Close">×</button></div><div className="modalActions"><button type="button" className="secondaryButton" onClick={() => void openMacroCalendar("upcoming", 1)}>Active & future</button><button type="button" className="secondaryButton" onClick={() => void openMacroCalendar("past", 1)}>View past events</button></div><div className="walletSettingsList">{macroEvents.length ? macroEvents.map((event) => <article key={event.id}><div><strong>{event.title}</strong><small>{new Date(event.startsAt).toLocaleString()} · {event.provider} · {event.impact}</small></div><div className="walletActions"><a className="secondaryButton" href={event.sourceUrl} target="_blank" rel="noreferrer">Source ↗</a></div></article>) : <p className="muted">{macroView === "past" ? "No past macro events are recorded." : "No active or future high-impact events are recorded."}</p>}</div><div className="modalActions"><button type="button" className="secondaryButton" disabled={saving || macroPage === 1} onClick={() => void openMacroCalendar(macroView, macroPage - 1)}>Previous</button><span className="muted">Page {macroPage}</span><button type="button" className="secondaryButton" disabled={saving || !macroHasMore} onClick={() => void openMacroCalendar(macroView, macroPage + 1)}>Next</button></div></section></div>}
  </>;
}
