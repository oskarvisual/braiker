...
type HistoryData = { bot: { id: string; name: string; lifeStatus: "ACTIVE" | "DEAD"; runMode: "OFF" | "SIMULATION" | "PAPER_ACTIVE"; status: string; killSwitch: boolean; adaptiveRiskEnabled: boolean; initialCapital: string; currentCapital: string; reservedCapital: string; openPositionCount: number; symbols: string[] }; orders: HistoryOrder[]; scans: AnalysisScan[]; adaptiveRiskAdjustments: AdaptiveRiskAdjustment[]; operatingCosts: OperatingCost[]; page: number; hasMore: boolean };
...
export function BotHistoryModal({ bot, onClose }: { bot: HistoryBot; onClose: () => void }) {
  ...
  const [tab, setTab] = useState<"operations" | "analysis" | "adaptive" | "chats">("operations");
  ...
  useEffect(() => {
    ...
    void Promise.all([fetch(`/api/bots/${bot.id}/history?page=1`), fetch(`/api/bots/${bot.id}/analysis?page=1`)])
      .then(async ([historyResponse, analysisResponse]) => {
        ...
        if (mounted) setData({ ...history, scans: analysis.scans ?? [], operatingCosts: history.operatingCosts ?? [], page: 1, hasMore: Boolean(history.hasMore || analysis.hasMore) });
      })
  ...
  const history = data?.bot ?? { id: bot.id, name: bot.name, lifeStatus: "ACTIVE" as const, runMode: "OFF" as const, status: "PAUSED", killSwitch: true, adaptiveRiskEnabled: false, initialCapital: "0", currentCapital: "0", reservedCapital: "0", openPositionCount: 0, symbols: [] };
