...
- `/manager` — Admin-only **Bot Manager**. It calls Alpaca’s exchange-aware clock for the operational context; failure is explicitly `UNKNOWN`. Exact bot names receive bounded orders/fills, risk decisions, scans, and positions. `Recordar para <nombre exacto>: <regla>` records versioned internal caution. `Evento macro: título | fecha ISO con zona | https://fuente` creates a HIGH event only where the host matches an active approved MACRO Resource. In bot chat, `Recordar: <regla>` does the same. Visible Additional instructions always take priority; internal learning can only caution/veto optional AI review and cannot create signals, change capital/risk/size, alter bot power, or submit orders.
