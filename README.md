...
- Admin-managed **Macro safety guard** persists official high-impact economic-release timestamps and source URLs. Settings has a paginated active/future modal plus immutable past history. It rejects only new `BUY` proposals during the global configured window (default/minimum 10 minutes before and 15 after; up to 240), while allowing risk-reducing `SELL`s.
