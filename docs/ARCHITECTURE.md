...
| Settings | `/settings`, `/api/wallets`, `/api/wallets/[walletId]/capital`, `/api/settings/paper-capital`, `/api/settings/synchronization`, `/api/settings/notifications`, `/api/settings/telegram/pairing`, `/api/settings/macro-events` | global Paper capital, serialized virtual-wallet budget changes, schedule, persisted alert preferences, per-event macro protection windows and immutable event history |
