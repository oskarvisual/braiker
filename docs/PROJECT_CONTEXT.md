...
- `/setup` — Admin bot listing and add/edit/clone/import modal. It separately shows current liquid capital and current asset value, exports one configuration-only package per bot, and imports only a validated, previewed package after a wallet and budget are manually selected. It shows a read-only survival indicator, risk limits, globally enabled symbols, and one ON/OFF switch. The editor offers an opt-in **Adaptive risk limits** checkbox; when enabled, it hides the three manual fields and permits only deterministic survival reductions inside their saved ceiling. The history modal adds **Assets** and **Adaptive risk** tabs. Assets are informational, valued only from persisted Alpaca global positions, and never silently use cost basis. Active bots can move capital to/from their wallet; dead bots expose history and cloning only. Clicking a bot name or eye icon opens its full-width history modal. Settings synchronizes Alpaca's active, tradable, non-OTC US-equity master list into separate paginated **Selected assets** and **Available assets** lists; each list searches on the server so a large refresh cannot hide an enabled symbol. A global disable removes that symbol from every bot, while a later enable leaves watchlists unassigned until an Admin selects it again.
