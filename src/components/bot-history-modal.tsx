...
type Performance = { startingCapital: string; netContributions: string; liquidCapital: string; assets: string | null; equity: string | null; realizedPnl: string; unrealizedPnl: string | null; tradingPnl: string | null; operatingCosts: string };
type PerformanceSnapshot = { marketDate: string; liquidCapital: string; assets: string; equity: string; capturedAt: string };
type HistoryData = { bot: { id: string; name: string; lifeStatus: "ACTIVE" | "DEAD"; runMode: "OFF" | "SIMULATION" | "PAPER_ACTIVE"; status: string; killSwitch: boolean; adaptiveRiskEnabled: boolean; initialCapital: string; currentCapital: string; reservedCapital: string; openPositionCount: number; symbols: string[]; assets: { value: string | null; valuedAt: string | null; unpricedSymbols: string[] }; assetPositions: AssetPosition[] }; performance: Performance; performanceHistory: PerformanceSnapshot[]; orders: HistoryOrder[]; scans: AnalysisScan[]; adaptiveRiskAdjustments: AdaptiveRiskAdjustment[]; operatingCosts: OperatingCost[]; page: number; hasMore: BotHistoryPageAvailability };

const money = (value: string | null) => value === null ? "Unavailable — sync Alpaca" : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(Number(value));
...
function PerformanceChart({ points }: { points: PerformanceSnapshot[] }) {
  if (points.length < 2) return <p className="muted">Daily equity history starts after Alpaca has completed enough valuations for this bot.</p>;
  ...
  return <div className="botPerformanceChart"><div className="chartScale"><span>{money(String(max))}</span><span>{money(String(min))}</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Bot daily equity"><polyline points={path} /></svg><div className="chartLabels"><span>{dateTime(points[0].capturedAt)}</span><span>{dateTime(points.at(-1)!.capturedAt)}</span></div></div>;
}
