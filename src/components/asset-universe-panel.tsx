...
export function AssetUniversePanel() {
  ...
  async function sync() { setSyncing(true); try { const response = await fetch("/api/settings/assets", { method: "POST" }); const body = await response.json() as { synced?: number; error?: string }; const completed = typeof body.synced === "number" ? body.synced : 0; setNotice(response.ok ? `${completed} Alpaca assets synchronized. New assets remain disabled until you select them.` : completed > 0 ? `${completed} assets were synchronized before the catalog became unavailable. ${assetUniverseFeedback(body.error)}` : assetUniverseFeedback(body.error)); if (response.ok || completed > 0) await Promise.all([loadPage(true, selectedSearch), loadPage(false, availableSearch)]); } catch { setNotice(assetUniverseFeedback(undefined)); } finally { setSyncing(false); } }
  ...
  const renderList = (title: string, enabled: boolean, search: string, setSearch: (value: string) => void, page: AssetPage) => {
    const needsInitialSync = !enabled && page.total === 0 && !search.trim();
    return <section className="assetUniverseGroup"><div className="assetUniverseGroupHeading"><div><h3>{title}</h3><p className="muted">{page.total} {page.total === 1 ? "asset" : "assets"}</p></div></div>{needsInitialSync ? <div className="assetEmptyState assetSyncEmptyState"><strong>No assets have been synchronized yet.</strong><p>Load the current Alpaca Paper-tradable US equities catalog to choose more assets for bots.</p>{notice ? <p className="assetSyncStatus" role="status">{notice}</p> : null}<button type="button" className="secondaryButton" onClick={() => void sync()} disabled={syncing}>{syncing ? "Synchronizing…" : "Sync assets from Alpaca"}</button></div> : <><label className="assetSearch">Search {title.toLowerCase()}<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Symbol, name, or exchange" /></label><div className="assetUniverseList">{page.assets.map((asset) => <label key={asset.symbol}><input type="checkbox" checked={asset.enabled} disabled={!asset.active || !asset.tradable} onChange={() => void toggle(asset)} /><span><strong>{asset.symbol}</strong><small>{asset.name} · {asset.exchange}</small></span></label>)}</div>{page.assets.length === 0 ? <p className="muted assetEmptyState">No matching assets.</p> : null}{page.nextCursor ? <button type="button" className="secondaryButton assetLoadMore" onClick={() => void loadPage(enabled, search, page.nextCursor, true)}>Load more</button> : null}</>}</section>;
  };
