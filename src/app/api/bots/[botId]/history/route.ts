...
export async function GET(request: Request, context: { params: Promise<{ botId: string }> }) {
  try {
    ...
    const [proposals, adjustmentRows, costRows, globalPositions, capitalEvents, fillTotals, performanceSnapshots] = await Promise.all([
      ...
      prisma.botPerformanceSnapshot.findMany({ where: { botId }, orderBy: { marketDate: "asc" }, take: 90, select: { marketDate: true, liquidCapital: true, assetValue: true, equity: true, capturedAt: true } })
    ]);
    ...
    return NextResponse.json({
      ...
      performance,
      performanceHistory: performanceSnapshots.map((snapshot) => ({ marketDate: snapshot.marketDate.toISOString(), liquidCapital: snapshot.liquidCapital.toString(), assets: snapshot.assetValue.toString(), equity: snapshot.equity.toString(), capturedAt: snapshot.capturedAt.toISOString() })),
