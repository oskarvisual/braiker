...
type HistoryData = { bot: { id: string; name: string; lifeStatus: "ACTIVE" | "DEAD"; initialCapital: string; currentCapital: string; reservedCapital: string; openPositionCount: number; symbols: string[] }; orders: HistoryOrder[]; scans: AnalysisScan[] };
...
export function BotHistoryModal({ bot, onClose }: { bot: HistoryBot; onClose: () => void }) {
  ...
  const [tab, setTab] = useState<"operations" | "analysis">("operations");
  ...
  const history = data?.bot ?? { name: bot.name, lifeStatus: "ACTIVE" as const, initialCapital: "0", currentCapital: "0", reservedCapital: "0", openPositionCount: 0, symbols: [] };
