...
export async function GET(_: Request, context: { params: Promise<{ botId: string }> }) {
  try {
    ...
    return NextResponse.json({
      bot: { id: bot.id, name: bot.name, templateId: bot.templateId, avatarSeed: bot.avatarSeed, lifeStatus: bot.lifeStatus, runMode: bot.runMode, killSwitch: bot.killSwitch, currentCapital: bot.currentCapital.toString(), initialCapital: bot.initialCapital.toString(), symbols: bot.watchlist.map((item) => item.symbol) },
