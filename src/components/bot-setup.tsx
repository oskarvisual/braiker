...
type Bot = { id: string; name: string; templateId: string; avatarSeed: string; runMode: string; status: string; lifeStatus: "ACTIVE" | "DEAD"; initialCapital: string; currentCapital: string; killSwitch: boolean; symbols: string[]; customInstructions: string; riskPolicy: { maxPositionSize: string; maxDailyLoss: string; maxTradesPerDay: number }; lastAnalysis: { startedAt: string; status: string; message: string } | null };
...
type HistoryData = { bot: Pick<Bot, "id" | "name" | "templateId" | "avatarSeed" | "lifeStatus" | "runMode" | "killSwitch" | "currentCapital" | "initialCapital" | "symbols">; orders: HistoryOrder[]; scans: AnalysisScan[] };
...
export function BotSetup({ initialWallets, templates }: { initialWallets: Wallet[]; templates: Template[] }) {
  ...
  const [historyTab, setHistoryTab] = useState<"operations" | "analysis">("operations");
  ...
  async function save() {
    ...
    if (create) setWallets((current) => current.map((wallet) => wallet.id === walletId ? { ...wallet, unallocatedCapital: body.walletUnallocatedCapital, bots: [...wallet.bots, { ...body, symbols: selectedSymbols, customInstructions, riskPolicy: body.riskPolicy, lastAnalysis: null }] } : wallet));
  ...
  async function openHistory(bot: Bot) {
    setHistoryBot(bot); setHistoryData(null); setHistoryTab("operations"); setHistoryLoading(true);
    const [response, analysisResponse] = await Promise.all([fetch(`/api/bots/${bot.id}/history`), fetch(`/api/bots/${bot.id}/analysis`)]);
    const body = await response.json();
    const analysisBody = analysisResponse.ok ? await analysisResponse.json() : { scans: [] };
    ...
    setHistoryData({ ...body, scans: analysisBody.scans ?? [] });
  }
      ...
      <section className="modalCard botModal" role="dialog" aria-modal="true" aria-labelledby="bot-modal-title">
        ...
        <form ref={formRef} onSubmit={preventImplicitSave}>
          ...
          {step === 3 && <>
            ...
            <fieldset className="riskLimits"><legend>Bot risk limits <small>May be lower than the {activeTemplate.name} profile, never higher.</small></legend><label>Max position (USD)<input type="number" min="0.01" step="0.01" max={activeTemplate.riskPolicy.maxPositionSize} value={maxPositionSize} onChange={(event) => setMaxPositionSize(event.target.value)} required /></label><label>Max daily loss (USD)<input type="number" min="0.01" step="0.01" max={activeTemplate.riskPolicy.maxDailyLoss} value={maxDailyLoss} onChange={(event) => setMaxDailyLoss(event.target.value)} required /></label><label>Max trades / day<input type="number" min="1" step="1" max={activeTemplate.riskPolicy.maxTradesPerDay} value={maxTradesPerDay} onChange={(event) => setMaxTradesPerDay(event.target.value)} required /></label></fieldset>
          </>}
