...
- `/bots/[botId]` and the third **Chats** tab in bot history — local-only, user/bot-isolated contextual sessions. They are locked until the bot is `PAPER_ACTIVE`, `RUNNING`, alive, and not Kill-Switched. They see bounded safe data for that bot’s watchlist, scans, daily context, and proposals, and cannot alter bot configuration, capital, risk, power, Kill Switches, or orders. A deliberately prefixed `Important:` / `Importante:` message creates auditable, bounded caution for the current New York market day only; it is separate from the immutable resource briefing and can only defer/block the optional advisory.
