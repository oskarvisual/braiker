import { defaultMacroGuardWindow } from "./macro-guard-settings";

export type MacroCalendarEvent = { id: string; title: string; startsAt: Date; impact: string; sourceUrl: string; beforeMinutes?: number; afterMinutes?: number };
...
export function activeMacroGuard(events: MacroCalendarEvent[], now = new Date()): MacroGuard {
  const event = events
    ...
    .filter((candidate) => now.getTime() >= candidate.startsAt.getTime() - (candidate.beforeMinutes ?? defaultMacroGuardWindow.beforeMinutes) * 60_000 && now.getTime() <= candidate.startsAt.getTime() + (candidate.afterMinutes ?? defaultMacroGuardWindow.afterMinutes) * 60_000)
