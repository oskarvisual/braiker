...
| History | `/activity`, `/bots/[botId]` | order lifecycle filtering and per-bot order history across all wallets accessible to the signed-in user; internal orders are linked by immutable bot id, so a bot rename does not affect attribution; unlinked Alpaca snapshots remain explicitly external |
