  ...
  return <div className="modalOverlay" role="presentation">
    <section className="modalCard botHistoryModal" role="dialog" aria-modal="true" aria-labelledby="history-modal-title">
      ...
      {loading ? <p className="muted modalLoading">Loading this bot's history…</p> : data && <>
        ...
        <section className="survivalPanel" aria-label="Survival status"><BotSurvivalStatus state={survival} /><div><strong>Survival is informational</strong><p>The bot's permanent survival rules, risk limits, Kill Switch, and trading controls remain unchanged.</p></div><dl><div><dt>Liquid oxygen</dt><dd>{money(history.currentCapital)}</dd></div><div><dt>Reserved</dt><dd>{money(history.reservedCapital)}</dd></div><div><dt>Open positions</dt><dd>{history.openPositionCount}</dd></div></dl></section>
