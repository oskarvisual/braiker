...
| Bot Manager | `/manager`, `/api/manager/sessions`, `/api/manager/sessions/[sessionId]/messages`, `/api/manager/actions` | Admin-only operational chat. It calls Alpaca clock per context and supplies bounded exact-bot history. Manager and bot-chat message APIs return a newest bounded page plus a cursor for older messages; the client transcript owns its scroll position. An optimistic browser message is reconciled against its matching persisted message after a reply, so it cannot remain rendered twice. Explicit `system report` / `reporte de sistema` / `/status` / `/report` is handled deterministically, reads the same sanitized `getSystemStatus` dependencies plus a read-only daily report window, and never calls the model or mutates state beyond its chat transcript. Confirmed exact-name ON/OFF remains the only power mutation. Explicit internal-learning and approved-source macro commands are Admin-only, fully audited, and cannot affect strategy/risk/execution. |
